...
# --- Player Search ---
all_players = sorted(set(batting_df["Name"].dropna().unique()) | set(bowling_df["Player Name"].dropna().unique()))
selected_player = st.selectbox("Select a player", options=all_players)

if selected_player:
    view_type = st.radio("Choose Stats Type", ["Batting Stats", "Bowling Stats"], horizontal=True)
